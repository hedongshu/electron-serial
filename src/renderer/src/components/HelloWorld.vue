...
export default {
    name: 'Main',
    watch: {
        autoSendRate(val: number) {
            let _ts = this;
            if (val > 0) {
                _ts.autoSendRate(_ts.autoSend);
            }
        }
    },
    data() {
    ...
    methods: {
        openCom() {
            ...
            port.on('data', function (data: { toString: (arg0: string | undefined) => string; }) {
                // 判断是否显示接收的数据
        ...
        portWrite() {
            ...
            if (port && port.isOpen && _ts.pushData) {
                ...
                port.write(pushData, encoding, function (err: string, result: string) {
                    if (err) {
        ...
        genRenderer() {
            ...
            ipcRenderer.on('saveOptions', (event: any) => {
                if (event) {
                    _ts.saveOptions();
                }
            });
            ipcRenderer.on('loadOptions', (event: any) => {
                if (event) {
                    _ts.loadOptions();
                }
            });
            ipcRenderer.on('reloadPorts', (event: any) => {
                if (event) {
                    _ts.genPorts();
                }
            });
            ipcRenderer.on('writeFile', (event: any) => {
                if (event) {
                    _ts.writeFile();
                }
            });
            ipcRenderer.on('readFile', (event: any) => {
                if (event) {
                    _ts.readFile();
                }
            });
        ...
        strToBinary(str: string, binary: number) {
            let result = [];
            let list = str.split("");
        ...
        read(title: string, method: string, filters: { name: string; extensions: string[]; }[]) {
            let _ts = this;
            ...
            dialog.showOpenDialog(win, {
                ...
                filters: filters
            }).then((result: { canceled: any; filePaths: PathLike[]; }) => {
                if (result.canceled) {
                    return;
                }
                // 创建可读流
        ...
        write(title: string, data: string, filters: { name: string; extensions: string[]; }[]) {
            const { dialog } = window.Electron.remote;
            const win = window.Electron.remote.getCurrentWindow();
            dialog.showSaveDialog(win, {
                title: title,
                filters: filters
            }).then((result: { canceled: any; filePath: PathLike; }) => {
                if (result.canceled) {
                    return;
                }
